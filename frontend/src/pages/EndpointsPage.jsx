...
import GenericSectionPage, {
  IcoOk, IcoWarn, IcoArrow, IcoChevronDown, IcoSliders, BarChart,
...
function DevicesSection({ devices, onGoToIntegrations }) {
  ...
  const filtered = useMemo(() => {
    ...
    return q ? devices.filter((d) => d.name.toLowerCase().includes(q) || d.ip.includes(q)) : devices;
  }, [devices, query]);
  ...
  return (
    <div className="sp-section sp-devices">
      ...
      <div className="sp-dev-tabs">
        <button className={`sp-dev-tab ${tab === "endpoints"     ? "sp-dev-tab--active" : ""}`} onClick={() => setTab("endpoints")}>Endpoints</button>
        <button className={`sp-dev-tab ${tab === "non-deployed"  ? "sp-dev-tab--active" : ""}`} onClick={() => setTab("non-deployed")}>Non-Deployed Endpoints</button>
      </div>
      ...
      {tab === "endpoints" ? (
        <>
          <div className="sp-devices__search-wrap">
            ...
            <input className="sp-devices__search" placeholder="Search by name or IP…" value={query} onChange={(e) => setQuery(e.target.value)}/>
            {query && <button className="sp-devices__search-clear" onClick={() => setQuery("")}>✕</button>}
          </div>
          <div className="sp-dev-table">
            <div className="sp-dev-table__head">
              <span>Endpoint</span><span>IP Address</span><span>OS</span>
              <span>Group</span><span>Last Seen</span><span>Status</span>
            </div>
                ...
                return (
                  <div key={dev.id} className="sp-dev-row">
                    ...
                    <span className={`sp-dev-row__status ${isOk ? "sp-dev-row__status--ok" : "sp-dev-row__status--off"}`}>
                      <span className="sp-dev-row__dot" />{isOk ? "OK" : "Inactive"}
                    </span>
                  </div>
                );
              })}
        ...
        <div className="sp-nondeployed">
          ...
          <div className="sp-nondeployed__desc">
            No connection to Microsoft Entra ID (Azure AD) or local Active Directory has been detected. Connect a directory to automatically discover unprotected devices.
          </div>
...
export default function EndpointsPage() {
  ...
  const section = data?.sections.find((s) => s.id === "endpoints");
  if (!section) return <div className="sp-center">Endpoints section not found.</div>;
