...
const REGIME_CYCLE = ["realistic", "none", "all_yellow", "all_red"];
...
export default function Dashboard() {
  ...
  const cycleTest = async () => {
    ...
    try {
      const response = await fetch(`${AGENT_URL}/api/regime/${mode}`, { method: "POST" });
  ...
  const okCount = displaySections.filter((s) => s.status === "ok" || s.status === "safe").length;
    ...
    <div className="dashboard">
      ...
      <header className="dashboard__header">
        ...
        <div className="dashboard__stats">
          ...
          <button className="dashboard__refresh-btn" onClick={refresh} title="Refresh">↺</button>
        </div>
      </header>
      ...
      <div className="dashboard__body">
        <div className="dashboard__ring-col">
          ...
          <p className="dashboard__ring-hint">Hover a segment for details · Click to drill in</p>
        </div>
      ...
      <section className="dashboard__issues">
        ...
        <div className="dashboard__issues-list">
          {displaySections.flatMap((s) => s.issues).length === 0 ? (
            <div className="dashboard__no-issues">No active issues — all systems secure</div>
          ) : (
            displaySections
              .flatMap((s) => s.issues.map((issue) => ({ ...issue, sectionLabel: s.label, sectionId: s.id })))
              .sort((a, b) => {
